...
class CheckPoint(object):
    ...
    def save_model(self, model, best_flag=False, index=0, tag=""):
        ...
        if best_flag:
            if tag != "":
                model_path = os.path.join(self.save_path, "%s_best_model.pth"%tag)
            ...
            if tag != "":
                model_path = os.path.join(self.save_path, "%s_model_%03d.pth" % (tag, index))
                torch.save(model, model_path)
            else:
                model_path = os.path.join(self.save_path, "model_%03d.pth" % index)
                torch.save(model, model_path)
        return model_path
